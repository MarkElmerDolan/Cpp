...
int main(int argc, char **argv){
	...
	while( buffer.compare( "exit") != 0){
		...
		tempCommand.isPipedOut = false;
		tempCommand.isPipedIn = false;
...
void printCommandInfo(command **com){
	...
	while(com[i] != NULL){
		...
		if(com[i]->isPipedOut == true){
			cout << "program is outputting to a pipe" << endl;
		}
		if(com[i]->isPipedIn == true){
			cout << "program is reading from a pipe" << endl;
		}
		i++;
	}
}
	...
	while(com[i] != NULL){
		if(com[i]->isPipedOut == true){ // next command isPipedIn should be true
			...
			if(pid == 0){
				...
				pid = fork();
				if(pid == 0){
					int newstdin = open("namedPipe", O_RDONLY);
